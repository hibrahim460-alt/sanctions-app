...
# means a broken fetch rather than a genuinely empty list. The floor can be
# overridden per source via the source config (see sources.py min_records),
# because e.g. the OFAC SDN list is huge while the non-SDN consolidated list
# is only a few hundred entries.
CONTRACTS = {
    # OFAC SDN and Consolidated share the same XML structure (sdnEntry).
    "OFAC_SDN": {
        "record_tag": "sdnEntry",
        "min_records": 1000,
        "required_fields": ["uid"],
...
def post_parse_check(source, records, prev_count=None, min_records=None):
    ...
    # Count floor — source config can override the contract default.
    floor = min_records if min_records is not None else contract.get("min_records", 1)
    if n == 0:
        return {"status": "fail",
                "issues": ["Parser produced 0 records — likely a schema change or failed fetch"]}
    if n < floor:
        issues.append(
            f"Only {n} records parsed (expected at least ~{floor} for this list). "
            f"If this is the OFAC non-SDN consolidated list, a few hundred is normal — "
            f"lower the floor for this source. Otherwise it may be a partial fetch or schema change.")
        status = "warn"
...
def validate_feed(source, raw, records, prev_count=None, min_records=None):
    ...
    post = post_parse_check(source, records, prev_count, min_records=min_records)
